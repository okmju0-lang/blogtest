...
- Writer가 해당 카테고리 템플릿(`/.Codex/agents/writer/references/templates/`)을 참조하여 초고를 작성한다.
...
- 전달: 현재 draft 파일 경로 + `/.Codex/agents/brand-editor/references/brand-voice-guide.md` 경로
...
- 실행 명령:
  ```
  python .Codex/skills/image-generator/scripts/generate_image.py batch \
...
- Visual Editor가 수행하는 작업:
  ...
  - `/.Codex/agents/visual-editor/scripts/capture_screenshot.py` 실행.
  - 출력: `output/posts/{post_id}/images/screenshot_{n}.png`
  - 반드시 출처 URL을 캡션으로 명시한다.

  10b. **다이어그램 전문화**
  - 기존 `.mmd` 파일의 스타일을 카테고리별 컬러 팔레트로 통일하고 전문적으로 개선한다.
  - `/.Codex/agents/visual-editor/scripts/enhance_diagram.py`로 스타일 강화 후 `diagram-renderer` 스킬로 재렌더링.
...
- 인포그래픽, 다이어그램, 차트처럼 **이미지 안에 텍스트가 포함되는 시각물은 한글을 1순위**로 사용한다.
- 영문은 2순위로 허용한다. 제품명, 고유명사, 업계 약어(AI, API, CRM 등), 또는 가독성을 위해 꼭 필요한 경우에만 제한적으로 사용한다.
- Mermaid 다이어그램 라벨도 동일한 원칙을 따른다. 가능하면 한글 라벨을 쓰고, 혼합 표기가 더 자연스러운 경우에만 영문을 병기한다.
...
서브에이전트는 `/.Codex/agents/{agent-name}/AGENT.md`의 지침에 따라 동작한다.
...
> **리드 CTA 규칙**: 모든 블로그 포스트 하단부에 리드 CTA를 삽입한다. 기본 위치는 본문 종료 직후이며, 참고 자료 섹션이 있다면 그 앞에 둔다.
> CTA 문구는 카테고리별 템플릿(`references/templates/`)에 정의된 것을 사용한다.
> 인용 블록(`>`) 형식으로 작성하여 본문과 시각적으로 구분한다.
> CTA 바로 앞에는 자동 구분선(`---`)을 넣지 않는다. CTA는 본문 흐름의 자연스러운 연장선으로 배치한다.

> **본문 마크다운 포맷 규칙**: 본문 가독성을 위해 Writer는 아래 형식을 기본값으로 사용한다.
> 문단 사이는 반드시 한 줄 이상 비우고, 한 문단은 2~4문장 중심으로 끊어 읽기 쉽게 작성한다.
> 첫 번째를 제외한 주요 `##` 섹션 앞에는 `---`를 넣어 본문 구간을 구분한다.
> 목록, 인용문, 표, 이미지 앞뒤에는 빈 줄을 두어 블록 간 호흡을 확보한다.
