...
- 스크립트 동작 흐름:
  ...
  2. 마크다운 본문을 HTML로 변환
  3. 스티비 API로 이메일 생성 (`POST /v2/emails`)
  4. HTML 콘텐츠 설정 (`POST /v2/emails/{id}/content`)
  5. 즉시 발송 (`POST /v2/emails/{id}/send`) 또는 예약 발송 (`POST /v2/emails/{id}/reserve`)
...
| 스티비 이메일 발송 (11) | HTTP 200 응답 + 이메일 ID 반환 | 1회 재시도 후 에스컬레이션. API 키 오류(401/403) 시 즉시 중단 + 사용자 안내 |
...
3. **이메일 발송 승인** (워크플로우 2 단계 11): 스티비 이메일 발송 전 담당자에게 발송 여부(즉시/예약/스킵) 확인.
...
| `visual_report.md` 존재 | 단계 10 (비주얼 강화 완료) | 단계 11 (스티비 이메일 발송) |
