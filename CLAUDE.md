...
- `AX 훈련/` 폴더가 없거나, 관련 자료가 비어 있거나, 이번 주제에 쓸 만한 파일이 없다면 사용자가 업로드한 임의의 예시 사례를 대체 1차 소스로 사용한다.
...
- 사용자가 업로드한 예시 사례를 fallback으로 사용할 때도 같은 원칙을 적용한다. 즉, 사례 맥락과 관찰을 우선 정리하고, AI 일반론처럼 단정하지 않는다.
