...
| `GOOGLE_AI_API_KEY` 설정 | `.env` 읽기 | 이미지 생성 불가 — 사용자에게 경고 후 이미지 단계 스킵 가능 |
...
8a. **이미지 프롬프트 작성**
- `draft_final.md`와 카테고리를 읽고 `image_spec.md`를 작성한다.
- 카테고리별 이미지 요구사항 참조 (하단 이미지 생성 규칙 섹션).
- 각 이미지: `image_id`, `type`, `placement`, `prompt`, `aspect_ratio`, `alt_text` 포함.
- `schema-validator`로 필수 필드 검증. 실패 시 1회 재시도.
- 출력: `output/drafts/{post_id}/image_spec.md`

8b. **AI 이미지 생성** (thumbnail, illustration 타입)
- `image-generator` 스킬 호출. image_spec.md의 각 항목을 개별 호출.
- 출력: `output/drafts/{post_id}/images/{image_id}.png`
- API 오류 시 최대 2회 재시도.
...
- **thumbnail / illustration**: `image-generator` 스킬 (Nano Banana 2 / Gemini API)
  - 썸네일: 1920×1080 (16:9)
  - 본문 삽입: 1024×1024 (1:1) 또는 4:3
- **diagram**: LLM이 Mermaid 코드 직접 작성 → `diagram-renderer` 스킬로 검증 + 렌더링

### 프롬프트 작성 가이드

- 글의 핵심 메시지와 시각적으로 연결되는 이미지를 만든다.
- 텍스트가 들어가는 이미지는 피한다 (AI 이미지의 텍스트 오류 위험).
- 카테고리별 스타일 방향(`/.claude/skills/image-generator/references/image-style-guide.md`) 참조.
...
| `image-generator` | image_spec.md의 thumbnail/illustration 항목 | Orchestrator |
