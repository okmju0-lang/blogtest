...
INFOGRAPHIC_MARKERS = (
    "infographic", "diagram", "chart", "flowchart", "timeline",
    "comparison", "callout", "label", "legend", "table", "matrix",
    "framework", "process map", "process diagram", "data visualization",
    "인포그래픽", "다이어그램", "도표", "차트", "흐름도", "타임라인",
    "비교표", "레이블", "범례", "콜아웃", "프로세스 맵", "데이터 시각화",
)

KOREAN_TEXT_RULE = (
    "If the image includes any visible text, labels, captions, headings, callouts, "
    "legends, UI copy, or numbers with units, prefer natural Korean Hangul first. "
...
def enforce_korean_text_prompt(prompt: str) -> str:
    """인포그래픽/다이어그램 계열 프롬프트에 한글 텍스트 규칙을 덧붙인다."""
    lowered = prompt.lower()
    if any(marker in lowered for marker in TEXTLESS_MARKERS):
        return prompt
    if not any(marker in lowered for marker in INFOGRAPHIC_MARKERS):
        return prompt
    if re.search(r"\b(korean|hangul)\b", prompt, re.IGNORECASE) or "한국어" in prompt or "한글" in prompt:
        return prompt
    return f"{prompt.strip()} {KOREAN_TEXT_RULE}"
...
def generate_image(prompt: str, image_type: str, output_path: str) -> dict:
    ...
    effective_prompt = enforce_korean_text_prompt(prompt)
