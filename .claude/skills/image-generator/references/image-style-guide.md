...
| 스타일 키 | 색감 | 적합한 상황 |
|---|---|---|
| `graphic-recording` | 와인+골드+오렌지, 크림 배경 | 따뜻한 톤, 교육적 콘텐츠, 핸드드로잉 느낌 |
...
**같은 글 내부**: `illustration_1`과 `illustration_2`는 반드시 **같은 스타일 키**를 사용한다.
- 프롬프트 내용(시각화 대상)은 달라도 되지만, 스타일 프리픽스는 동일하게 적용한다.

**글 간 연속 사용 금지**: 직전 포스트와 같은 스타일을 연속으로 사용하지 않는다.
- `output/infographic_style_registry.md`를 확인하여 직전 포스트의 스타일을 파악한다.
- 포스트 완료 후 Orchestrator가 레지스트리를 업데이트한다.
